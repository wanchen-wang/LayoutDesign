...
def resolve_data_root(user_input=None):
    """解析数据根目录，优先使用用户输入，否则自动探测常见位置"""
    if user_input:
        candidate = os.path.abspath(user_input)
        if os.path.isdir(candidate):
            return candidate
        raise FileNotFoundError(f"指定的数据目录不存在: {candidate}")
    ...
    raise FileNotFoundError("未找到数据目录，请使用 --data-root 指定已有数据目录")
...
if __name__ == "__main__":
    ...
    if total == 0:
        print(f"在目录中没有可用的数据组: {data_root}")
        sys.exit(1)

    if args.group:
        if args.group not in groups:
            print(f"指定的数据组不存在: {args.group}")
            print(f"可用数据组数量: {total}")
            sys.exit(1)
    ...
    path = os.path.join(data_root, selected_group)
    print(f"正在加载已有数据组: {selected_group}")
