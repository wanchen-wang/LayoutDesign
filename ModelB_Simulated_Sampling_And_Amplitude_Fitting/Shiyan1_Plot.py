"""
Single_W_A_Lagrangian_Hybrid.py 绘图模块
可视化混合模式采样：前半段受拉格朗日影响，后半段恢复不受影响
"""
...
from Shiyan1 import run_single_hybrid
...
def plot_hybrid_sampling(t_array, w_isw_array, depth_obs, mode_array, t_meet, thermocline_depth, error_pct):
    """
    可视化混合模式采样：
    ...
    if len(mode_array) > 0:
        ...
        lagrangian_end_idx = np.where(mode_changes == -1)[0]
        
        for start_idx in lagrangian_start_idx:
            start_time = t_array[start_idx + 1]
            ax1.axvline(start_time, color='orange', linestyle='--', linewidth=1.5, alpha=0.6)
            ax1.text(start_time + 10, max(w_isw_array)*0.6, 'Enter Lagrangian', color='orange', fontsize=10, rotation=90)
        
        for end_idx in lagrangian_end_idx:
            end_time = t_array[end_idx + 1]
            ax1.axvline(end_time, color='green', linestyle='--', linewidth=1.5, alpha=0.6)
            ax1.text(end_time + 10, max(w_isw_array)*0.4, 'Exit Lagrangian', color='green', fontsize=10, rotation=90)
        
        # 高亮拉格朗日影响区域背景
        for i in range(len(lagrangian_start_idx)):
            start = t_array[lagrangian_start_idx[i] + 1] if i < len(lagrangian_start_idx) else t_array[0]
            end = t_array[lagrangian_end_idx[i] + 1] if i < len(lagrangian_end_idx) else t_array[-1]
    ...
    ax1.set_title(f'Hybrid Lagrangian Sampling (Error: {error_pct:.2f}%)', fontsize=14)
...
if __name__ == "__main__":
    ...
    try:
        result = run_single_hybrid(path, return_full=True)

        print("\n绘制混合模式采样图...")
        plot_hybrid_sampling(
            result['t_array'],
            result['w_isw_array'],
            result['depth_obs'],
            result['mode_array'],
