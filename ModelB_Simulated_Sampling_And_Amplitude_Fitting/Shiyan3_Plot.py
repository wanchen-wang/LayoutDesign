"""
Shiyan3.py 绘图模块
可视化实验组3采样：前半段不受拉格朗日影响，后半段受水平流和垂直流影响
"""
...
from Shiyan3 import run_single_shiyan3
...
def plot_shiyan3_sampling(t_array, w_isw_array, depth_obs, mode_array, t_meet, thermocline_depth, error_pct):
    """
    可视化实验组3采样：
    ...
    ax1.set_title(f'Shiyan3: Post-Lagrangian Sampling (Error: {error_pct:.2f}%)', fontsize=14)
...
if __name__ == "__main__":
    ...
    try:
        result = run_single_shiyan3(path, return_full=True)

        print("\n绘制实验组3采样图...")
        plot_shiyan3_sampling(
            result['t_array'],
            result['w_isw_array'],
            result['depth_obs'],
            result['mode_array'],
