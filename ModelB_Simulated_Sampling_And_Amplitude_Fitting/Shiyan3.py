...
DEFAULT_RESULTS_DIR = Path(__file__).resolve().parent / "Analysis_Results_Shiyan3"


def run_single_shiyan3(data_dir, return_full=False):
    """
    实验组3采样：前半段不受拉格朗日影响，后半段受水平流和垂直流影响。
    
    逻辑：
    1. 默认状态：不受水流影响，按标准轨迹运动
    2. 当检测到垂直水流由负变正时：进入拉格朗日影响模式
       - 下降速度加快（w_obs_real = w_stdy + w_isw）
       - 水平位置受水平流影响（u_obs_real = v_g + u_bg）
...
def batch_process(base_data_dir=DEFAULT_V_WAVE_DATA_DIR):
    print(f"\n{'='*60}")
    print(f"🚀 启动实验组3采样仿真（前半段不受影响，后半段受拉格朗日影响）...")
    print(f"{'='*60}")
    ...
    for i, folder_name in enumerate(wave_folders, 1):
        ...
        try:
            res = run_single_shiyan3(data_dir)
    ...
    if results_list:
        ...
        output_filename = DEFAULT_RESULTS_DIR / "analysis_results_shiyan3.csv"
