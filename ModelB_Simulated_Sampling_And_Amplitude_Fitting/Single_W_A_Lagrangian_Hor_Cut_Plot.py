...
def run_single_cut_hor(data_dir, cut_pct):
    """Run one Lagrangian sampling with horizontal current and apply dynamic cut threshold.

    Parameters
    ----------
    data_dir : str
        Path to one subdirectory in V_Wave_Data_Hor.
    cut_pct : float
    ...
    interp_u = RegularGridInterpolator(
        (x_grid, z), U_Vel_xz, bounds_error=False, fill_value=0.0
    )
...
def plot_cut_result_hor(result, cut_pct, group_name):
    ...
    title = (
        f"Single W-A Lagrangian Hor-Cut Plot | Group: {group_name} | Cut: {cut_pct:.1f}%\n"
...
if __name__ == "__main__":
    ...
    if not groups:
        print("No available group folders found under V_Wave_Data_Hor.")
        sys.exit(1)
    ...
    try:
        result = run_single_cut_hor(group_path, cut_pct)
        plot_cut_result_hor(result, cut_pct, group_name)
