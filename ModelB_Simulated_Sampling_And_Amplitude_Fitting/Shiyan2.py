...
DEFAULT_RESULTS_DIR = Path(__file__).resolve().parent / "Analysis_Results_Shiyan2"


def run_single_shiyan2(data_dir, return_full=False):
    """
    实验组2采样：前半段受拉格朗日影响（只受垂直水流），后半段恢复不受影响状态采样。
    
    逻辑：
    1. 默认状态：不受水流影响，按标准轨迹运动
    2. 当检测到垂直水流 w_isw < 0（负瓣区域）时：进入拉格朗日影响模式
       - 下降速度加快（w_obs_real = w_stdy + w_isw）
       - 在同样时间会到达比预定深度更深的位置
    3. 当检测到垂直水流由负变正时：恢复不受影响模式，不考虑水流影响
...
def batch_process(base_data_dir=DEFAULT_V_WAVE_DATA_DIR):
    print(f"\n{'='*60}")
    print(f"🚀 启动实验组2采样仿真（前半段拉格朗日影响，后半段恢复）...")
    print(f"{'='*60}")
    ...
    for i, folder_name in enumerate(wave_folders, 1):
        ...
        try:
            res = run_single_shiyan2(data_dir)
    ...
    if results_list:
        ...
        output_filename = DEFAULT_RESULTS_DIR / "analysis_results_shiyan2.csv"
