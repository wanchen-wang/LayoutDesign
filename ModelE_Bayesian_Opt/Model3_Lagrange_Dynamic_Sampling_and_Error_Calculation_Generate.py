"""
调用 process_18cut 函数处理 V_Wave_Data_Hor 中的所有流场数据
...
V_WAVE_DATA_ROOT = PROJECT_ROOT / "ModelA_Virtual_Internal_Solitary_Wave_Data_Generation" / "V_Wave_Data_Hor"
MODEL3_ANALYSIS_DIR = MODULE_DIR / "Analysis_Bayesian_Opt_Model3_Hor_Data"
...
def _process_one_dir(args):
    ...
    try:
        required_files = ['params.json', 'x_grid.npy', 'y_grid.npy', 'z.npy', 'W_Vel_3D.npy', 'U_Vel_3D.npy', 'W_profile.npy']
        ...
        result_dict = process_18cut(w_c_threshold, V_target, zeta_target, f_s, run_data_dir, return_diagnostic_data=True)
...
def main():
    ...
    if not os.path.exists(v_wave_data_root):
        print(f"错误：V_Wave_Data_Hor 目录不存在: {v_wave_data_root}")
        return
    ...
    if results:
        ...
        output_csv = MODEL3_ANALYSIS_DIR / f"Model3_Hor_18cut_Comparison_{w_c_threshold:.3f}_{V_target:.2f}_{zeta_target:.1f}_{f_s:.1f}.csv"
