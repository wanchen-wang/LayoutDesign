...
def plot_18cut_result(result, group_name):
    df = result["df"]
    ...
    ax1.axhline(
        ...
        label=f"18% cut threshold ({cutoff_val:.4f} m/s)"
    )
    ...
    ax1.fill_between(
        ...
        label="Kept for integration (18% cut)"
    )
    ...
    title = (
        f"18%-Cut Lagrangian Sampling (Hor) | Group: {group_name}\n"
        f"w_thresh={result['w_c_threshold']:.3f}m/s  V_target={result['V_target']:.2f}m/s  "
        f"zeta_target={result['zeta_target']:.1f}deg  f_s={result['f_s']:.1f}Hz\n"
        f"Delta_h_calc={result['Delta_Z_calc']:.2f}m  true h0={result['Delta_Z_true']:.2f}m  "
        f"J={result['J']:.4f}m  (doppler x {result['doppler_factor']:.3f})"
    )
    ax1.set_title(title)
...
if __name__ == "__main__":
    ...
    if not groups:
        print("未在 V_Wave_Data_Hor 下找到任何子文件夹。")
        sys.exit(1)
    ...
    zeta_target = _get_float("zeta_target (deg, 如 -35.0): ")
    ...
    print(f"\n正在运行仿真: {group_name} ...")
    try:
        result = process_18cut(w_c_threshold, V_target, zeta_target, f_s, group_path,
                               return_diagnostic_data=True)
        plot_18cut_result(result, group_name)
